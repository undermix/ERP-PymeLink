...
const PaymentForm: React.FC<PaymentFormProps> = ({ client, unpaidInvoices, onSave, onCancel }) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (name === 'invoiceId') {
            ...
            if (selectedInvoice) {
                const amountToPay = selectedInvoice.total;
                setFormData(prev => ({ ...prev, amount: amountToPay }));
                if (formData.method === PaymentMethod.Check) {
                    setCheckData(prev => ({...prev, amount: amountToPay}));
                }
            }
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <div>
                <label htmlFor="invoiceId" className="block text-sm font-medium text-gray-600 mb-1">Orden a Pagar</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div>
                    <label htmlFor="amount" className="block text-sm font-medium text-gray-600 mb-1">Monto del Pago</label>
                 ...
                 <div>
                    <label htmlFor="date" className="block text-sm font-medium text-gray-600 mb-1">Fecha de Pago</label>
            ...
            {formData.method === PaymentMethod.Check && (
                <div className="space-y-4 rounded-lg border border-gray-200 p-4 bg-gray-50">
                    <h3 className="font-semibold text-gray-700">Detalles del Cheque</h3>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label htmlFor="checkNumber" className="block text-sm font-medium text-gray-600 mb-1">Número de Cheque</label>
                        ...
                        <div>
                            <label htmlFor="checkBank" className="block text-sm font-medium text-gray-600 mb-1">Banco</label>
                     ...
                     <div>
                        <label htmlFor="checkPaymentDate" className="block text-sm font-medium text-gray-600 mb-1">Fecha de Cobro</label>
            ...
            <div className="flex justify-end space-x-4 pt-4">
                ...
                <button type="submit" className="px-6 py-2 bg-blue-600 text-white font-medium text-sm rounded-md hover:bg-blue-700 transition-colors">Guardar Pago</button>
            </div>
        </form>
    );
};
